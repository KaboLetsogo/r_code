...
wheat.colors <- ifelse(test = wheat$type=="Healthy", yes = "black",
                       no = ifelse(test = wheat$type=="Sprout", yes = "red", no = "cyan"))
...
legend(x = 6.15, y = 0.75, legend = c("Healthy", "Sprout", "Scab"), lty = c("solid", "longdash", "dotdash"),
       col=c("black", "red", "cyan"), cex=0.8, bty="n")
#legend(locator(1), legend = c("Healthy", "Sprout", "Scab"), lty = c("solid", "longdash", "dotdash"),
#  col=c("black", "red", "cyan"), cex=0.8, bty="n")  #Original legend using locator(1)
...
order(wheat$size)  # 269 has the 2nd largest size

# Alternative version of plot using GGally package (this is built on
#   the ggplot2 package). Note that each variable is standardized
#   by  default so the numerical scale is different than for parcoord().
#   The argument scale = "uniminmax" gives the same scaling as parcoord().
library(package = GGally)
wheat.g <- data.frame(kernel = 1:nrow(wheat), wheat[,2:7],
                      class.new = ifelse(test = wheat$class == "hrw", yes = 0, no = 1))
ggparcoord(data = wheat.g, columns = c(1:6,8), groupColumn = 7, scale = "uniminmax")
...
legend(x = 6.15, y = 0.75, legend=c("Healthy", "Sprout", "Scab", "Kernel 269"), lty = c("solid", "longdash", "dotted", "dotted"),
       col=c("black", "red", "cyan", "purple"), cex=0.75, bty="n", lwd = c(1, 1, 1, 10)) 
...
legend(x = 7.15, y = 0.75, legend=c("Healthy", "Sprout", "Scab"), lty=c(1,1,1), col=c("black", "red", "cyan"), cex=1, bty="n")

# Another way to do these plots with brushing (highlight parts of plot to change colors)
#   Need to have Java install on computer.
library(package = iplots)
ipcp(wheat3[order(wheat.colors2),])


###################################################################################
# Principal component analysis in order to plot in 3D
...
legend(x = 2, y = -2, legend=c("Healthy", "Sprout", "Scab"), pch = c(1,1,1),
       col=c("black", "red", "cyan"), cex=1, bty="n") 
...
######################################
# Estimate model using multinom() and perform hypothesis tests
...
# Check out what is inside mod.fit
names(mod.fit)
...
# Wald tests for one beta at a time - usually not done since it tests only one
#   parameter at a time rather than all parameters corresponding to an explanatory
#   variable
...
# LRT for class
mod.fit.Ho <- multinom(formula = type ~ density + hardness + size + weight + moisture,
                       data = wheat)
anova(mod.fit.Ho, mod.fit) # class alone
library(package = car)
Anova(mod.fit)  # class and other variables
qchisq(p = 0.95, df = 2)
...
###############################################################################
# Estimate model using vglm() from the VGAM (Vector Generalized Additive Models) package

library(package = VGAM)

# This will match mod.fit with multinom()
#   Note that multinomial(refLevel = 1) works as well
mod.fit.v <- vglm(formula = type ~ class + density + hardness + size + weight + moisture,
                  family = multinomial(refLevel = "Healthy"), data = wheat)
summary(mod.fit.v)  # Wald tests given in output
summary(mod.fit.v, lrt0 = TRUE)  # LRTs given in output
# While LRTs in the output are nice, it is for only one parameter at a time
#   rather than for all parameters representing a variable.


# Use a stricter convergence criterion to determine if parameter estimates change
#   Four additional arguments were used here: epsilon, maxit, stepsize, and trace.
#   The vglm.control() function help provides information about what they represent.
#   In summary, epsilon, maxit, and trace work in a similar manner to what was shown
#   for the glm() function. The stepsize argument controls how much the parameter estimates
#   change on successive iterations. The default value is 1. Lower values indicate
#   smaller changes, leading usually to a longer amount of time to reach convergence.
#   We used a few different values for epsilon and stepsize to make sure we were
#   satisfied that convergence was obtained.
mod.fit.v.strict <- vglm(formula = type ~ class + density + hardness + size + weight + moisture,
                         family = multinomial(refLevel = "Healthy"), data = wheat, epsilon = 1e-12,
                         maxit = 100, stepsize = 0.5, trace = TRUE)
summary(mod.fit.v.strict)
mod.fit.v.strict@criterion


# This is the default - uses the "last" level as the baseline
mod.fit.v2 <- vglm(formula = type ~ class + density + hardness + size + weight + moisture,
                   family = multinomial, data = wheat)
summary(mod.fit.v2)
# Same but now specify the level
mod.fit.v2 <- vglm(formula = type ~ class + density + hardness + size + weight + moisture,
                   family = multinomial(refLevel = "(Last)"), data = wheat)
# Change in version version 1.0-2 - refLevel = "(Last)" now rather than refLevel = "last"
summary(mod.fit.v2)


# Check out what is inside mod.fit.v
names(mod.fit.v)  # Does not provide useful information
slotNames(mod.fit.v)  # Need to use this function instead because S4 class system
attributes(mod.fit.v)  # EVERYTHING - a long list of items
names(attributes(mod.fit.v))  # Just list object names within it.


#Check out method functions available
class(mod.fit.v)
showMethods(classes = "vglm")  # All method functions for this class
showMethods(f = "predict")

# Better LRTs because it tests all parameters associated with an explanatory variable
anova(mod.fit.v)
Anova(mod = mod.fit.v) # INCORRECT output
...
#######################
# Program the equations

# Obtain observation values, ";" ends a command
x1 <- 0;          x2 <- wheat[1,2]; x3 <- wheat[1,3]
x4 <- wheat[1,4]; x5 <- wheat[1,5]; x6 <- wheat[1,6]

# To obtain the beta.hats, need to use the coefficient() function
#   There is not a coefficient component to the mod.fit object, unlike with glm() created objects
beta.hat <- coefficients(mod.fit)
den <- 1 + exp(beta.hat[1,1] + beta.hat[1,2]*x1 + beta.hat[1,3]*x2 + beta.hat[1,4]*x3
               + beta.hat[1,5]*x4 + beta.hat[1,6]*x5 + beta.hat[1,7]*x6) +
  exp(beta.hat[2,1] + beta.hat[2,2]*x1 + beta.hat[2,3]*x2 + beta.hat[2,4]*x3
      + beta.hat[2,5]*x4 + beta.hat[2,6]*x5 + beta.hat[2,7]*x6)
pi.hat.healthy <- 1/den
pi.hat.scab <- exp(beta.hat[1,1] + beta.hat[1,2]*x1 + beta.hat[1,3]*x2 + beta.hat[1,4]*x3
                   + beta.hat[1,5]*x4 + beta.hat[1,6]*x5 + beta.hat[1,7]*x6) / den
pi.hat.sprout <-  exp(beta.hat[2,1] + beta.hat[2,2]*x1 + beta.hat[2,3]*x2 + beta.hat[2,4]*x3
                      + beta.hat[2,5]*x4 + beta.hat[2,6]*x5 + beta.hat[2,7]*x6) / den
round(data.frame(pi.hat.healthy, pi.hat.scab, pi.hat.sprout), 4)

# Quicker way to compute pi^ using formulas
...
######################
# Better ways to compute estimated probabilities

mod.fit$fitted.values[1,]
mod.fit.v@fitted.values[1,]

# Use the method function for predict() and the multinom class
predict(object = mod.fit, newdata = wheat[1,], type = "probs")
# Predicted classification
predict(object = mod.fit, newdata = wheat[1,], type = "class")  # default

predict(object = mod.fit.v, newdata = wheat[1,], type = "response")


# Use emmeans package
library(package = emmeans)
calc.prob <- emmeans(object = mod.fit, specs = ~ type, mode = "prob",
                     at = wheat[1,1:6])
# pi.hat and CIs for pi, need df=Inf because a t-distribution is used otherwise
summary(object = calc.prob, df = Inf, level = 0.95)


# Using deltamethod() of the car package - while the emmeans package
#   is easier to use, this code helps readers see better how the delta method
#   is implemented.
# Parts of a character string needed to provide the model
...
curve(expr =  predict(object = mod.fit.nom.density, newdata = data.frame(density = x), type = "probs")[,1],
      add = TRUE, xlim = c(min(wheat$density[wheat$type == "Healthy"]), max(wheat$density[wheat$type == "Healthy"])), col = "black", lty = "solid", lwd = 2, n = 1000)
curve(expr = predict(object = mod.fit.nom.density, newdata = data.frame(density = x), type = "probs")[,2],
      col = "cyan", lty = "dotdash", lwd = 2, n = 1000, add = TRUE,
...
legend(x = 1.4, y = 0.8, legend=c("Healthy", "Sprout", "Scab"), lty=c("solid","longdash","dotdash"),
       col=c("black","red","cyan"), bty="n", lwd = c(2,2,2), seg.len = 4)


# Black-and-white version of plot
# pdf(file = "c:\\figures\\Figure3.3BW.pdf", width = 7, height = 6, colormodel = "cmyk")   # Create plot for book
curve(expr = predict(object = mod.fit.nom.density, newdata = data.frame(density = x), type = "probs")[,1], ylab = expression(hat(pi)), xlab = "Density",
...
curve(expr = predict(object = mod.fit, newdata = data.frame(class = "hrw", density = x,
                                                            hardness = mean(wheat$hardness), size = mean(wheat$size), weight = mean(wheat$weight),
                                                            moisture = mean(wheat$moisture)), type = "probs")[,2],
      xlim = c(min(wheat$density), max(wheat$density)), col = "cyan", lty = "dotdash", lwd = 2, n = 1000,
      add = TRUE)
curve(expr = predict(object = mod.fit, newdata = data.frame(class = "hrw", density = x,
                                                            hardness = mean(wheat$hardness), size = mean(wheat$size), weight = mean(wheat$weight),
                                                            moisture = mean(wheat$moisture)), type = "probs")[,3],
      xlim = c(min(wheat$density), max(wheat$density)), col = "red", lty = "longdash", lwd = 2, n = 1000,
      add = TRUE)
legend(x = 1, y = 1, legend=c("Healthy", "Sprout", "Scab"), lty=c("solid","longdash","dotdash"), col=c("black","red","cyan"), bty="n", lwd = c(2,2,2))
...
# Later in the program, we remove column 8 for this calculation
#   because there is an additional column is put into the data.frame
c.value <- c(1, sd.wheat)  # class = 1 is first value
round(c.value,2)

# beta.hat_jr for r = 1, ..., 6  and j = 2, 3
# mod.fit$coefficients does not exist, so use coefficients()
...
# Odds ratios from vglm()
exp(rep(c.value, each = 2) * mod.fit.v@coefficients[-c(1,2)])
...
# Wald CIs for OR using emmeans - density
calc.est <- emmeans(object = mod.fit, specs = ~ type + density, mode = "latent",
                    at = list(density = c(1.25, 1.25+0.1313021)))
test.info <- contrast(object = calc.est, interaction = list("revpairwise", "pairwise"))
confint(object = test.info, df = Inf, type = "response", level = 0.95)

# Wald CIs for OR using emmeans - class
#   Class values do not need to be specified in at because it has only two values
#   and emmeans detects this.
calc.est <- emmeans(object = mod.fit, specs = ~ type + class, mode = "latent")
test.info <- contrast(object = calc.est, interaction = list("revpairwise", "pairwise"))
confint(object = test.info, df = Inf, type = "response", level = 0.95)


# Profile LR intervals
conf.beta.v <- confint(object = mod.fit.v, level = 0.95, method = "profile")
round(conf.beta.v, 2)
round(exp(rep(c.value, each = 2) * conf.beta.v[-c(1,2),]), 2)
# Density only
data.frame(type = c("scab vs. healthy", "sprout vs. healthy"),
           low = round(1/exp(c.value[2] * conf.beta.v[5:6,2]), 2),
           up =  round(1/exp(c.value[2] * conf.beta.v[5:6,1]), 2), row.names = NULL)

# Odds ratios for scab vs. sprout should also be investigated.
#   The computations can be performed by re-estimating the model
#   with scab or sprout as the base category and then using the same
#   type of code as above. Alternatively, the emmeans package provides
#   ways to do these calculations with the original model's fit.
...
# Estimate model
#   Used more stringent convergence criteria to obtain a better match with vglm()
mod.fit.class.density <- multinom(formula = type ~ class*density, data = wheat,
                                  reltol = 1e-12)
coefficients(mod.fit.class.density)
summary(mod.fit.class.density)

mod.fit.class.density.v <- vglm(formula = type ~ class*density,
                                family = multinomial(refLevel = "Healthy"), data = wheat)
summary(mod.fit.class.density.v)

coefficients(mod.fit.class.density)
beta.hat2 <- coefficients(mod.fit.class.density)[1,-1]
beta.hat2

# Odds ratio at density = 1
density <- 1
exp(beta.hat2[1] + beta.hat2[3]*density)

# Odds ratio at density = 1 using emmeans
options(width = 60)
calc.est.inter <- emmeans(object = mod.fit.class.density, specs = ~ type + class,
                          at = list(density = 1), mode = "latent")
test.info.inter <- contrast(object = calc.est.inter, interaction = list("revpairwise", "revpairwise"))
confint(object = test.info.inter, df = Inf, type = "response",
        level = 0.95)
options(width = 80)

# Alternative way to use emmeans with contasts
calc.est.inter2 <- emmeans(object = mod.fit.class.density,
                           specs = ~ type + class + density, at = list(density = 1), mode = "latent")
find.order <- contrast(object = calc.est.inter2, method = "identity")
coefficients(find.order)

hrw.dens1 <- c(-1,  1,  0,   0,  0,  0)
srw.dens1 <- c( 0,  0,  0,  -1,  1,  0)
test.info.inter2 <- contrast(object = calc.est.inter2, type = "response",
                             method = list("Odds for hrw" = hrw.dens1, "Odds for srw" = srw.dens1,
                                           "OR for srw vs. hrw" = srw.dens1 - hrw.dens1))
confint(object = test.info.inter2, df = Inf, level = 0.95)
...
# Quadratic and interaction terms example
...
mod.fit.trans2 <- multinom(formula = type ~ class + density + I(density^2) +
                             density:class, data = wheat,
                           maxit = 1000, reltol = 1.0e-12)  # Converges and matches vglm()
summary(mod.fit.trans2)

mod.fit.trans1.v <- vglm(formula = type ~ class + density + I(density^2) +
                           density:class, family = multinomial(refLevel = "Healthy"), data = wheat)
summary(mod.fit.trans1.v)


# Make the last level the base level and use multinom()
...
######################################
# Estimate model using polr() and perform hypothesis tests

# Model with all explanatory variables
mod.fit.ord <- polr(formula = type.order ~ class + density + hardness + size + weight + moisture,
...
library(package = VGAM)  # If not done already

# options(width = 70)
mod.fit.ord.v <- vglm(formula = type.order ~ class + density + hardness + size + weight + moisture, data = wheat,
                      family = cumulative(parallel = TRUE))
summary(mod.fit.ord.v)
# Anova(mod.fit.ord.v) # INCORRECT
anova(mod.fit.ord.v)
# options(width = 80)

# To remove the warning message from summary() above, one can use the
#   the ordered() function as an alternative to factor() when ordering
#   the levels of the response.
wheat$type.order2 <- ordered(wheat$type, levels = c("Scab",  "Sprout", "Healthy"))
mod.fit.ord.v2 <- vglm(formula = type.order2 ~ class + density + hardness + size + weight + moisture, data = wheat,
                       family = cumulative(parallel = TRUE))
summary(mod.fit.ord.v2)
...
# Use object from polr()
predict(object = mod.fit.ord, newdata = wheat[1,], type = "probs")
# Use object from vglm()
predict(object = mod.fit.ord.v, newdata = wheat[1,], type = "response")

# Predicted classifications
head(predict(object = mod.fit.ord, type = "class"))

# Estimate pi_j without predict()
kern1 <- as.numeric(c(0, wheat[1,-c(1,7,8,9)]))  # Kernel #1
round(kern1,2)
...
# Use emmeans package
library(package = emmeans)
calc.prob <- emmeans(object = mod.fit.ord, specs = ~ type.order,
                     at = wheat[1,], mode = "prob")
summary(object = calc.prob, level = 0.95)


# Modifying deltaMethod() of the car package - while the emmeans package
#   is easier to use, this code helps readers see better how the delta method
#   is implemented.
...
# Uses deltaMethod.default() - not correct
deltaMethod(object = beta.hat, g = g.scab, vcov. = cov.mat)

# New function to find the interval
...
# Estimate model with density only
mod.fit.ord.dens <- polr(formula = type.order ~ density, data = wheat, method = "logistic")
summary(mod.fit.ord.dens)
...
#  Run the commented curve() code first if want only the proportional odds model
#curve(expr = predict(object = mod.fit.dens, newdata = data.frame(density = x), type = "probs")[,1], ylab = expression(hat(pi)), xlab = "Density",
#  xlim = c(min(wheat$density), max(wheat$density)), col = "black", lty = "solid", lwd = 2, n = 1000, type = "n",
#  panel.first = grid(col = "gray", lty = "dotted"))
lwd.po <- 4
curve(expr = predict(object = mod.fit.ord.dens, newdata = data.frame(density = x), type = "probs")[,3],
      col = "black", lty = "solid", lwd = lwd.po, n = 1000, add = TRUE,
      xlim = c(min(wheat$density[wheat$type == "Healthy"]), max(wheat$density[wheat$type == "Healthy"])))  # Healthy
curve(expr = predict(object = mod.fit.ord.dens, newdata = data.frame(density = x), type = "probs")[,1],
      col = "cyan", lty = "dotdash", lwd = lwd.po, n = 1000, add = TRUE,
      xlim = c(min(wheat$density[wheat$type == "Scab"]), max(wheat$density[wheat$type == "Scab"])))  # Scab
curve(expr = predict(object = mod.fit.ord.dens, newdata = data.frame(density = x), type = "probs")[,2],
      col = "red", lty = "longdash", lwd = lwd.po, n = 1000, add = TRUE,
      xlim = c(min(wheat$density[wheat$type == "Sprout"]), max(wheat$density[wheat$type == "Sprout"])))  # Sprout
legend(x = 1.4, y = 0.8, legend=c("Healthy", "Sprout", "Scab"), lty=c("solid","longdash","dotdash"),
       col=c("black","red","cyan"), bty="n", lwd = c(2,2,2), seg.len = 4)
...
curve(expr = predict(object = mod.fit.nom.density, newdata = data.frame(density = x), type = "probs")[,1],
      ylab = expression(hat(pi)), xlab = "Density",
      xlim = c(min(wheat$density), max(wheat$density)), col = "black", lty = "solid", lwd = 2, n = 1000, type = "n")
# Plot each pi_j
curve(expr = predict(object = mod.fit.nom.density, newdata = data.frame(density = x), type = "probs")[,1],
      col = "black", lty = "solid", lwd = 2, n = 1000, add = TRUE,
...
curve(expr = predict(object = mod.fit.ord.dens, newdata = data.frame(density = x), type = "probs")[,3],
      ...
      xlim = c(min(wheat$density[wheat$type == "Healthy"]), max(wheat$density[wheat$type == "Healthy"])))  # Healthy
curve(expr = predict(object = mod.fit.ord.dens, newdata = data.frame(density = x), type = "probs")[,1],
      col = "black", lty = "dotdash", lwd = lwd.po, n = 1000, add = TRUE,
      xlim = c(min(wheat$density[wheat$type == "Scab"]), max(wheat$density[wheat$type == "Scab"])))  # Scab
curve(expr = predict(object = mod.fit.ord.dens, newdata = data.frame(density = x), type = "probs")[,2],
      col = "black", lty = "longdash", lwd = lwd.po, n = 1000, add = TRUE,
      xlim = c(min(wheat$density[wheat$type == "Sprout"]), max(wheat$density[wheat$type == "Sprout"])))  # Sprout
legend(x = 1.4, y = 0.8, legend=c("Healthy", "Sprout", "Scab"), lty=c("solid","longdash","dotdash"),
       col=c("black","black","black"), bty="n", lwd = c(2,2,2), seg.len = 4)
# dev.off()  # Create plot for book
...
par(mfrow = c(3,1))
hist(wheat$density[wheat$type.order == "Scab"], xlim = c(0.5, 2.1), main = "Scab", col = "cyan", 
...
# Information about each variable to help with choosing c
summary(wheat)
sd.wheat <- apply(X = wheat[,-c(1,7,8,9)], MARGIN = 2, FUN = sd)
#  Need to remove column 8 here because type.order is now in wheat
c.value <-c(1, sd.wheat)
round(c.value, 2)  # class = 1 is first value


# OR using polr() results
round(exp(c.value*(-mod.fit.ord$coefficients)),2)
round(1/exp(c.value*(-mod.fit.ord$coefficients)),2)

# CIs using polr() results
#  Note that class() and methods() were used earlier in the program to see
#  there is a method function for confint().
...
# Using vglm() results
round(exp(c.value*(mod.fit.ord.v@coefficients[-c(1,2)])),2)
conf.beta.v <- confint(object = mod.fit.ord.v, level = 0.95, method = "profile")
conf.beta.v
round(exp(c.value*(conf.beta.v[-c(1,2),])),2)
...
beta.ci
confint.default(object = mod.fit.ord, parm = "density", level = 0.95)
vcov(mod.fit.ord.v)
confint(object = mod.fit.ord.v, parm = "density", level = 0.95, method = "wald")
# Small differences in the estimated covariance matrix lead to small differences in the intervals
#   Different convergence criteria lead to these small differences
round(rev(1/exp(beta.ci*c.value[2])),2)

# Wald using emmeans
# library(package = emmeans)  # If not done already
# options(width = 60)
calc.est <- emmeans(object = mod.fit.ord,
                    specs = ~ density,
                    at = list(density = c(1.25 + 0.13, 1.25)),
                    mode = "linear.predictor")
confint(object = contrast(object = calc.est, method = "revpairwise", type = "response"),
        level = 0.95)
# options(width = 80)

# This also works using the keyword "cut" to represent the response
#  categories to end the cumulative probabilities
calc.est <- emmeans(object = mod.fit.ord,
                    specs = ~ density | cut,
                    at = list(density = c(1.25 + 0.13, 1.25)),
                    mode = "linear.predictor")
confint(object = contrast(object = calc.est, method = "consec", type = "response"),
        level = 0.95)
...
mod.fit.ord.dens2 <- polr(formula = type.order ~ density + I(density^2), data = wheat, method = "logistic")
summary(mod.fit.ord.dens2)

mod.fit.ord.dens2.v <- vglm(formula = type.order ~ density + I(density^2), data = wheat,
                            family = cumulative(parallel = TRUE))
summary(mod.fit.ord.dens2.v)

# Use probit link function for proportional odds model with vglm()
mod.fit.ord.probit <- polr(formula = type.order ~ class + density + hardness + size + weight + moisture,
                           data = wheat, method = "probit")
summary(mod.fit.ord.probit)
mod.fit.ord.probit.v <- vglm(formula = type.order ~ class + density + hardness + size + weight + moisture, data = wheat,
                             family = cumulative(parallel = TRUE, link = probitlink))
summary(mod.fit.ord.probit.v)
...
# Using the clm() function in the ordinal package
library(package = ordinal)
mod.fit.ord3 <- clm(formula = type.order ~ class + density + hardness + size + weight + moisture, data = wheat)
class(mod.fit.ord3)
methods(class = clm)
mod.fit.ord3
